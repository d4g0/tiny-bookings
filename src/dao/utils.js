...
export function isValidHourTime2({ hour = 0, minute = 0 }) {
    ...
    const { h_error, h_value } = hourSchema.validate(hour, {
        presence: 'required',
        convert: false,
    });
    const { m_error, m_value } = minSchema.validate(minute, {
        presence: 'required',
        convert: false,
    });
...
export function isValidHourTimeInput(hourTime = { hours: 0, minutes: 0 }) {
    ...
    const { h_error, h_value } = hourSchema.validate(hourTime.hours, {
        presence: 'required',
        convert: false,
    });
    const { m_error, m_value } = minSchema.validate(hourTime.minutes, {
        presence: 'required',
        convert: false,
    });
...
export function isValidTimeZone(iana_time_zone) {
    ...
    const { error, value } = tzSchema.validate(iana_time_zone, {
        presence: 'required',
        convert: false,
    });
...
export function isValidRoomType(roomType) {
    ...
    var { error, value } = roomSchema.validate(roomType, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function isValidRoomAmenity(amenity) {
    ...
    var { error, value } = amenitySchema.validate(amenity, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function isValidRoomName(room_name) {
    ...
    var { error, value } = roomNameSchema.validate(room_name, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function isValidUserRoleKey(user_role_key) {
    ...
    const { error, value } = userRoleSchema.validate(user_role_key, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function isValidYearMonthDayDate(yearMonthDayDate = { year, month, day }) {
    ...
    const { y_error, y_value } = yearSchema.validate(yearMonthDayDate.year, {
        presence: 'required',
        convert: false,
    });
    const { m_error, m_value } = monthSchema.validate(yearMonthDayDate.month, {
        presence: 'required',
        convert: false,
    });
    const { d_error, d_value } = daySchema.validate(yearMonthDayDate.day, {
        presence: 'required',
        convert: false,
    });
...
export function isValidDateInput(dInput = { year, month, day, hour, minute }) {
    ...
    const { y_error, y_value } = yearSchema.validate(dInput.year, {
        presence: 'required',
        convert: false,
    });
    const { m_error, m_value } = monthSchema.validate(dInput.month, {
        presence: 'required',
        convert: false,
    });
    const { d_error, d_value } = daySchema.validate(dInput.day, {
        presence: 'required',
        convert: false,
    });
    const { h_error, h_value } = hourSchema.validate(dInput.hour, {
        presence: 'required',
        convert: false,
    });
    const { mi_error, mi_value } = minSchema.validate(dInput.min, {
        presence: 'required',
        convert: false,
    });

    complyAllSchemas =
        !y_error && !m_error && !d_error && !m_error && !d_error && !h_error && !m_error;
    return complyAllSchemas;
}

export function isValidBookingState(booking_state) {
    var stateSchema = Joi.string().required().trim().min(2).max(40);
    const { error, value } = stateSchema.validate(booking_state, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function isValidPaymentType(payment_type) {
    ...
    const { error, value } = paymentSchema.validate(payment_type, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function isValidCurrnecy(currency_key) {
    ...
    const { error, value } = currnecySchema.validate(currency_key, {
        presence: 'required',
        convert: false,
    });
    return !error;
...
export function utcDate({ year = 0, month = 0, day = 0, hour = 0, minute = 0 }) {
